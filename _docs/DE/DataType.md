...
- Interval Data의 0이라는 척도는 존재하지 않음을 의미하는 것이 아니라  그 값 자체로 인식해야 합니다. 
    - ex) 시간 00:00, 온도 0도
...
명목형 데이터(Nominal data)는 값에 레이블을 지정하거나 설명하는 데 사용됩니다. 
순서형 데이터(Ordinal data)는 데이터의 특정 순서에 대한 정보를 제공하는 데 사용되며 주로 만족도 조사에서 볼 수 있습니다. 
구간 데이터(Interval data)는 순서와 차이점을 이해하는 데 사용됩니다. 
비율 데이터(Radio data)는 동일성, 순서 및 차이에 대한 정보와 함께 각 데이터 내의 수치적 세부 사항에 대한 분석을 제공합니다.
